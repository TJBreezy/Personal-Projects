...
function searchLinks(searchTerm) {
    const searchTitlesChecked = searchTitles.checked;
    const searchUrlsChecked = searchUrls.checked;
    
    if (!searchTitlesChecked && !searchUrlsChecked) {
        renderLinks(); // Show all links if no search options selected
        return;
    }
    ...
    // Gather links based on current category
    if (currentCategory === 'All') {
        Object.values(links).forEach(categoryLinks => {
            if (categoryLinks) {
                linksToShow = linksToShow.concat(categoryLinks);
            }
        });
    } else {
        linksToShow = links[currentCategory] || [];
    ...
    // Filter and sort links based on search term
    const filteredLinks = linksToShow.filter(link => {
        const titleMatch = searchTitlesChecked && 
            link.title.toLowerCase().includes(searchTerm.toLowerCase());
        const urlMatch = searchUrlsChecked && 
    ...
    filteredLinks.forEach(link => {
        ...
        const linkContent = document.createElement('a');
        linkContent.href = link.url;
        linkContent.innerHTML = searchTitlesChecked 
            ? highlightText(link.title, searchTerm)
            : link.title;
        if (currentCategory === 'All') {
            linkContent.innerHTML += ` (${link.category})`;
        }
        linkContent.target = '_blank';
        linkElement.appendChild(linkContent);
...
searchTitles.addEventListener('change', () => {
    debouncedSearch(searchInput.value);
});

searchUrls.addEventListener('change', () => {
    debouncedSearch(searchInput.value);
});
